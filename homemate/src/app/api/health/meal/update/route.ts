...
const isValidMealValue = (value: unknown) =>
...
const buildUpdates = (updates: Record<string, unknown>) => {
  return MEAL_FIELDS.reduce<Record<string, unknown>>((accumulator, key) => {
    if (Object.prototype.hasOwnProperty.call(updates, key)) {
      const value = updates[key];
      if (value !== undefined) {
        accumulator[key] = normalizeText(value);
      }
    }
    return accumulator;
  }, {});
...
export async function POST(request: Request) {
  ...
  for (const [key, value] of Object.entries(cleanedUpdates)) {
    if (!MEAL_FIELDS.includes(key) || !isValidMealValue(value)) {
      return NextResponse.json(
  ...
  const { data: weekPlanData, error: weekPlanError } = await supabase
    ...
    .select("*, meal_day_plans(*)")
    .eq("user_id", userData.user.id)
    .eq("week_start_date", weekStart)
    .order("date", { ascending: true, foreignTable: "meal_day_plans" })
    .maybeSingle();
  ...
  if (updatedError) {
    return NextResponse.json(
      { error: "Failed to load updated meal plan" },
      { status: 500 }
    );
  }
  ...
  const { meal_day_plans: dayPlans, ...weekPlan } = updatedData;
